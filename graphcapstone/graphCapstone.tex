...
\end{lem}
\begin{proof}
Suppose that we construct a graph $G$ and basis $W$
\end{proof}
...
\node[main node] (1) {};
\node[main node] (2) [above right of=1] {};
\node[main node] (3) [above left of=1] {x};
...
\node[main node] (8) [right of=2] {};
\node[main node] (6) [below right of=8]{x};
...
\begin{lem}
 let $G$ be a unicyclic graph. Let $L$ be the cycle in $G$. Let $H = G-L$ then metric dimension of $G \leq$ metric dimension of $MD(H)\-$
\end{lem}
...
\end{proof}
\begin{lem}
 Let $G$ be a graph with a cut vertex $v$ and basis $W$. If $deg(v)>2$ or if for two components $H_1, H_2$ of $G-v$,
 $H_1$ has a vertex of degree $>$ 2, $W\nsubseteq H_2$.
\end{lem}
\begin{proof}
...
\bibliography{mybibjkl}  % add references to the list of references saved in the file mybib.bib
